...
class MessageDBHelper (context: Context) : DBHelper(context){
    ...
    companion object{
        ...
        private const val SQL_CREATE_MESSAGE_TABLE =
            "CREATE TABLE IF NOT EXISTS $MESSAGE_TABLE_NAME ($COLUMN_ID INTEGER PRIMARY KEY AUTOINCREMENT,$COLUMN_USER TEXT,$COLUMN_MESSAGE TEXT,$COLUMN_TIME INTEGER,$COLUMN_AVATAR_FILE_NAME TEXT)"
    ...
    fun getMessageByUser(user: String) : List<MessageModel>{
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $MESSAGE_TABLE_NAME WHERE $COLUMN_USER = '$user'", null)
        val messageList = ArrayList<MessageModel>()
        with(cursor){
            while (moveToNext()){
                val messageModel = MessageModel(
    ...
    fun updateMessageItem(messageModel: MessageModel): Int{
        ...
        val mInt = db.update(MESSAGE_TABLE_NAME, values, "$COLUMN_ID = ?", arrayOf(messageModel.id.toString()))
        db.close()
        return mInt
    }
